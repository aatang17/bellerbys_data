...
3. Open the **Settings** tab:
   ...
   - **Build Command:** leave default (Railway will run `pip install -r requirements.txt` if it sees `requirements.txt`).
   - **Start Command:** leave blank so Railway uses the **Procfile** in the repo root (`web: uvicorn app:app --host 0.0.0.0 --port $PORT`).
...
| Root Directory | (blank) | `Offer_Letter_Generator` |
